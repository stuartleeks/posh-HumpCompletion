...
Describe "PoshHumpTabExpansion2 - command completion" {
    ...
    It "provides matches filtered to prefix" {
        (PoshTabExpansion2Wrapper "Set-C").CompletionMatches | Should -Be @('Set-Content') # i.e. doesn't match "Command"
    }
    It "matches multiple items (including partial matches)" {
        # TODO - want to have this ordered by exact hump match first!
        #,(PoshHumpTabExpansion "Get-C") | Should -Be @('Get-Content', 'Get-Command', 'Get-ChildItem', 'Get-CimInstance')
        (PoshTabExpansion2Wrapper "Get-C").CompletionMatches | Should -Be @('Get-ChildItem', 'Get-CimInstance', 'Get-Command', 'Get-Content')
    }
    It "matches with lower-case filter refinement" {
        # TODO - want to have this ordered by exact hump match first!
        #,(PoshHumpTabExpansion "Get-C") | Should -Be @('Get-Content', 'Get-Command', 'Get-ChildItem', 'Get-CimInstance')
        (PoshTabExpansion2Wrapper "Get-ChI").CompletionMatches | Should -Be @('Get-ChildItem')
    }
    It "matches multiple items - multihump (including partial matches)" {
        (PoshTabExpansion2Wrapper "Get-CI").CompletionMatches | Should -Be @('Get-ChildItem', 'Get-CimInstance')
    }
    ...
    It "matches case-insensitively on Verb" {
        (PoshTabExpansion2Wrapper "set-C").CompletionMatches | Should -Be @('Set-Content')
    }
...
Describe "PoshHumpTabExpansion2 - parameter completion" {
    ...
    It "handles simple completion" {
        (PoshTabExpansion2Wrapper "Get-Help -Fu").CompletionMatches | Should -Be @("-Full", "-Functionality")
    }
    It "matches with hump completion on capitals" {
        (PoshTabExpansion2Wrapper "Get-Foo1 -TT").CompletionMatches | Should -Be @("-TestTwo", "-TestThree")
    }
    It "matches with hump completion on capitals and lowercase" {
        (PoshTabExpansion2Wrapper "Get-Foo1 -TTw").CompletionMatches | Should -Be @("-TestTwo")
    }
    It "matches in the middle of the command text" {
        ...
        $result.CompletionMatches | Should -Be @("-Full", "-Functionality")
    }
    It "matches with hump completion on capitals" {
        ...
        $result.ReplacementIndex | Should -Be 9
        $result.ReplacementLength | Should -Be 3
        $result.CompletionMatches | Should -Be @("-TestTwo", "-TestThree")
    }
}
Describe "PoshHumpTabExpansion2 - variable completion" {
    ...
    It "handles simple completion" {
        ...
        $result.CompletionMatches | Should -Be @("`$poshHumpCompletionTest_TestBar", "`$poshHumpCompletionTest_TestBaz")
        $result.ReplacementIndex | Should -Be 0
